...
const TeacherForgotPasswordForm = ({ onSwitch }) => {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      // change here to send the email 
      await axios.post(
        `${process.env.REACT_APP_API_URL}/api/teacher/forgot-password/send-otp`,
        { email: email } // Ensure email is correctly formatted
      );
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/api/teacher/forgot-password/verify-otp`,
        { email: email, otp: otp } // Ensure email and otp are correctly formatted
      );
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      await axios.post(
        `${process.env.REACT_APP_API_URL}/api/teacher/forgot-password/reset-password`,
        { email: email, otp: otp, newPassword: newPassword } // Ensure all fields are correctly formatted
      );
      ...
      {step === 1 && (
        <form onSubmit={handleEmailSubmit} className="w-full max-w-lg">
          <div className="mb-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full p-3 pl-10 border rounded-lg focus:outline-none focus:ring-red-500 focus:border-red-500"
              />
            </div>
          </div>
          ...
          <p className="text-sm text-gray-600 mt-4 text-center">
            Remembered?{" "}
            <button className="text-red-500 font-bold hover:underline" onClick={onSwitch}>
              Login
            </button>
          </p>
        </form>
...
export default TeacherForgotPasswordForm;
