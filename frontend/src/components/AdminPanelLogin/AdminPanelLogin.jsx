    ...
    <div className="flex flex-col lg:flex-row min-h-screen font-poppins">
      ...
      <div className="flex flex-col items-center justify-center p-8 w-full lg:w-1/2">
        <div className="w-full max-w-md">
          ...
          {error && <div className="bg-red-100 text-red-600 p-3 mb-4 rounded-lg text-center">{error}</div>}
          {loading ? (
            <Loading />
          ) : (
            <Formik initialValues={{ email: "", password: "" }} validationSchema={validationSchema} onSubmit={handleSubmit}>
              {({ isSubmitting }) => (
                <Form className="space-y-4">
                  ...
                  <button type="submit" disabled={isSubmitting} className="w-full bg-indigo-700 text-white p-3 rounded-lg hover:bg-indigo-900 transition duration-300 mt-6">
                    Sign In
                  </button>
                </Form>
              )}
